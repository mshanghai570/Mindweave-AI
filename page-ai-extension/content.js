// content.js — injected into every page, builds the floating panel UI.
(function () {
  'use strict';
  ...
  const DEFAULTS = {
    pai_apiKey: '',
    pai_baseUrl: 'https://api.openai.com/v1',
    pai_model: 'gpt-4o-mini',
    pai_temperature: '0.4',
  };
  ...
  async function getCfg() {
    ...
    return {
      apiKey: stored.pai_apiKey,
      baseUrl: stored.pai_baseUrl,
      model: stored.pai_model,
      temperature: stored.pai_temperature,
    };
  }

  async function setCfg(cfg) {
    await chrome.storage.local.set({
      pai_apiKey: cfg.apiKey,
      pai_baseUrl: cfg.baseUrl,
      pai_model: cfg.model,
      pai_temperature: cfg.temperature,
    });
  }
  ...
  // ---------- API call, routed through the background worker ----------
  function askAI(messages, onDone, onError) {
    getCfg().then((cfg) => {
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (chrome.runtime.lastError) {
            onError(chrome.runtime.lastError.message || 'Extension messaging error');
            return;
          }
          if (!response || !response.ok) {
            onError((response && response.error) || 'Unknown error');
            return;
          }
  ...
  panel.querySelector('#pai-cfg-save').addEventListener('click', async () => {
    const cfg = {
